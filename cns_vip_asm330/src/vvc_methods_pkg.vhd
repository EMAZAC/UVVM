...
package vvc_methods_pkg is
  ...
  --===============================================================================================
  -- Types and constants for the asm330 VVC
  --===============================================================================================
  constant C_VVC_NAME : string := "ASM330_VVC";
  ...
  shared variable shared_asm330_vvc_config       : t_vvc_config_array(0 to C_MAX_VVC_INSTANCE_NUM - 1)       := (others => C_SPI_VVC_CONFIG_DEFAULT);
  shared variable shared_asm330_vvc_status       : t_vvc_status_array(0 to C_MAX_VVC_INSTANCE_NUM - 1)       := (others => C_VVC_STATUS_DEFAULT);
  shared variable shared_asm330_transaction_info : t_transaction_info_array(0 to C_MAX_VVC_INSTANCE_NUM - 1) := (others => C_TRANSACTION_INFO_DEFAULT);

  -- Scoreboard
  package asm330_sb_pkg is new bitvis_vip_scoreboard.generic_sb_pkg
    generic map(t_element         => std_logic_vector(C_VVC_CMD_DATA_MAX_LENGTH - 1 downto 0),
                element_match     => std_match,
                to_string_element => to_string);
  use asm330_sb_pkg.all;
  shared variable asm330_VVC_SB : asm330_sb_pkg.t_generic_sb;
  ...
  procedure asm330_set_test_vectors(
    signal   VVCT                   : inout t_vvc_target_record;
    constant vvc_instance_idx       : in integer;
    constant test_vectors           : in t_asm330_test_vectors;
    constant msg                    : in string;
    constant scope                  : in string                   := C_VVC_CMD_SCOPE_DEFAULT
    );

  procedure asm330_set_spi_seq(
    signal   VVCT                   : inout t_vvc_target_record;
    constant vvc_instance_idx       : in integer;
    constant spi_cmd_seq            : in t_asm330_spi_cmd_seq;
    constant msg                    : in string;
    constant scope                  : in string                   := C_VVC_CMD_SCOPE_DEFAULT
    );

  procedure asm330_update_reg_map(
    signal   VVCT                   : inout t_vvc_target_record;
    constant vvc_instance_idx       : in integer;
    constant reg_map                : in t_asm330_reg_map;
    constant msg                    : in string;
    constant scope                  : in string                   := C_VVC_CMD_SCOPE_DEFAULT
    );
...
package body vvc_methods_pkg is
  ...
  begin
    ...
    shared_vvc_cmd.test_vectors(v_num_words to C_VVC_CMD_MAX_TEST_VECTOR_LENGTH-1) := (others => C_asm330_test_vector_element_def);
  ...
  begin
    ...
    shared_vvc_cmd.spi_cmd_seq(v_num_words to C_VVC_CMD_MAX_SPI_CMD_SEQ_LENGTH-1) := (others => C_asm330_spi_cmd_def);
